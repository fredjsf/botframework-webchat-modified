...
var HistoryView = (function (_super) {
    ...
    HistoryView.prototype.doCardAction = function (type, value, title, index) {
        //only disable old postback or imback buttons
        if (type == 'postBack' || type == 'imBack') {
            //only enable button for the last 2 activities
            var enableOldButtons = window.enableOldButtons || false;
            //default last 2 message bubble buttons enabled
            var lastIndexDisableButtons = window.lastIndexDisableButtons || 2;
            if (enableOldButtons) {
                this.props.setFocus();
                return this.props.doCardAction(type, value, title);
            }
            else {
                if (index >= window.chatbotMessageContentLength - lastIndexDisableButtons) {
                    this.props.setFocus();
                    return this.props.doCardAction(type, value, title);
                }
            }
    ...
    HistoryView.prototype.render = function () {
        ...
        if (this.props.size.width !== undefined) {
            if (this.props.format.carouselMargin === undefined) {
                ...
                //store in window
                window.chatbotMessageContentLength = this.props.activities.length;
                content = this.props.activities.map(function (activity, index) {
                    return React.createElement(WrappedActivity, { format: _this.props.format, key: 'message' + index, activity: activity, showTimestamp: index === _this.props.activities.length - 1 || (index + 1 < _this.props.activities.length && suitableInterval(activity, _this.props.activities[index + 1])), selected: _this.props.isSelected(activity), fromMe: _this.props.isFromMe(activity), onClickActivity: _this.props.onClickActivity(activity), onClickRetry: function (e) {
                        ...
                        React.createElement(ActivityView_1.ActivityView, { format: _this.props.format, size: _this.props.size, activity: activity, onCardAction: function (type, value, title) { return _this.doCardAction(type, value, title, index); }, onImageLoad: function () { return _this.autoscroll(); } }));
                });
            }
        }
        /**for custom LOADING**/
        var loadingClass = Chat_1.classList('wc-message-groups', !content || content.length == 0 ? 'loading-active' : '');
        return (React.createElement("div", { className: loadingClass, ref: function (div) { return _this.scrollMe = div || _this.scrollMe; } },
            React.createElement("div", { className: 'webchat-loading' },
                React.createElement("div", { className: "fw-style-spinner" },
