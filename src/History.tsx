...
import { Activity, Message, User, CardActionTypes } from 'botframework-directlinejs-modified';
...
export class HistoryView extends React.Component<HistoryProps, {}> {
    ...
    private doCardAction(type: CardActionTypes, value: string | object, title:string, index:number) {
        //only disable old postback or imback buttons
        if(type == 'postBack' || type == 'imBack'){
          //only enable button for the last 2 activities
          let enableOldButtons = window.enableOldButtons || false;
          //default last 2 message bubble buttons enabled
          let lastIndexDisableButtons = window.lastIndexDisableButtons || 2;
          if(enableOldButtons){
            this.props.setFocus();
    ...
    render() {
                ...
                content = this.props.activities.map((activity, index) =>
                    ...
                    >
                        <ActivityView
                            ...
                            activity={ activity }
                            onCardAction={ (type: CardActionTypes, value: string | object, title:string) => this.doCardAction(type, value, title, index) }
        ...
        /**for custom LOADING**/
        let loadingClass = classList(
          'wc-message-groups',
          !content || content.length == 0 ? 'loading-active' : ''
        );

        return (
            <div className={loadingClass} ref={ div => this.scrollMe = div || this.scrollMe }>
              <div className='webchat-loading'>
                <div className="fw-style-spinner">
